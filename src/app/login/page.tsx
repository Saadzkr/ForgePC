...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res?.error) { setError(_('login.invalid')); playError(); setLoading(false) }
    ...
    <div className="min-h-screen bg-black flex items-center justify-center px-6">
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="text-[0.6rem] tracking-[0.25em] text-muted-foreground uppercase">{_('login.title')}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-[0.55rem] tracking-wider uppercase text-muted-foreground block mb-1.5">{_('login.email')}</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="glass-input text-sm w-full px-3 py-2 rounded-lg" placeholder={_('login.email.placeholder')} required />
          </div>
          <div>
            <label className="text-[0.55rem] tracking-wider uppercase text-muted-foreground block mb-1.5">{_('login.password')}</label>
          ...
          >
            {loading ? _('login.loading') : _('login.button')}
          </button>
        </form>
